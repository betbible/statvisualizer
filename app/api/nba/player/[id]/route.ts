...
// Returns aggregate gamelogs and average stats for the given NBA player
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  const playerId = Number(params.id);
  const url = new URL(req.url);
  const season = url.searchParams.get('season')!;
  const windowSize = Number(url.searchParams.get('window'));
  const teamParam = url.searchParams.get('team')!; // 'all' or ID as string
  const period = url.searchParams.get('period')!; // 'Regular' or 'Playoff'
